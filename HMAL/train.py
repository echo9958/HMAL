...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        model = HMALDetector(nc=nc, anchors=hyp.get('anchors')).to(device)
    ...
    if pretrained and isinstance(ckpt, dict):
        ...
        if epochs < start_epoch:
            logger.info('%s has been trained for %g epochs. Fine-tuning for %g additional epochs.' %
                        (weights, ckpt.get('epoch', 0), epochs))
...
if __name__ == '__main__':
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        opt.weights, opt.resume, opt.batch_size, opt.global_rank, opt.local_rank = \
